...
public:
    ...
    for (unsigned int d=0; d<dim; ++d)
      {
        ...
        h2_inv[d] = (n_points_per_dim[d]+1) * (n_points_per_dim[d]+1)/
  ...
  void apply(const Vector<Number> &src,
        ...
        if (mpi_neighbors[2*d+side] < n_mpi_ranks)
          {
            ...
            if (my_size[0] * my_size[1] != data_to_send[2*d+side].size())
              {
                std::cout << "Internal error with dimension mismatch!" << std::endl;
                std::abort();
              }
      ...
      for (unsigned int iy = 0; iy < (dim > 1 ? sizes[1] : 1); ++iy)
        {
          ...
          if (dim > 1)
            dst_ptr[0] += hy * (2. * src(base_idx) -
                                (iy > 0 ? src(base_idx-strides[1]) : 0.) -
                                (iy < sizes[1] - 1 ? src(base_idx+strides[1]) : 0.));
          if (dim > 2)
            dst_ptr[0] += hz * (2. * src(base_idx) -
                                (iz > 0 ? src(base_idx-strides[2]) : 0.) -
...
#pragma omp simd
            for (unsigned int i = base_idx + 1; i < base_idx + sizes[0] - 1; ++i)
              {
          ...
          // of if statements and make the loop vectorizable
          else
            {
              for (unsigned int ix = 1; ix < sizes[0] - 1; ++ix)
                dst_ptr[ix] = hx * (2 * src(base_idx+ix)
                                    - src(base_idx+ix+1)
                                    - src(base_idx+ix-1));
          ...
          for (unsigned int ix = 0; ix < sizes[0]; ++ix)
            {
              ...
              values[count++] = 2.*h2_inv[0] + (dim > 1 ? (2.*h2_inv[1] +
                                                           (dim > 2 ? 2.*h2_inv[2] : 0.)) : 0.);
              if (ix < sizes[0] - 1)
                {
                  col_indices[count] = index + 1;
                  values[count++] = -h2_inv[0];
              ...
              for (unsigned int ix = 0; ix < (dim > 1 ? my_size[0] : 1); ++ix)
                {
                  ...
                  sparse.add_ghost_entry(iy*my_stride[1] + ix*my_stride[0] + offset,
                                         n_ghost_entries++,
                                         -h2_inv[d]);
                }
          }
