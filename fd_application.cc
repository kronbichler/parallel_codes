              ...
              const bool      do_sparse)
{
  ...
  if (my_mpi_rank == 0)
    std::cout << "Computing on a " << N_x << " x " << N_y << " x " << N_z
              << " domain with " << domain_partitions[0] << " x "
  ...
  std::vector<Number>           lower_left({0., 0., 0.});
  std::vector<Number>           upper_right({1., 1., 1.});
...
#pragma omp critical
...

      {
        ...
        if (my_mpi_rank == 0)
          std::cout << "Sparse matrix of size " << src.size() << " : "
                    ...
                    << 1e-9 * n_repeat *
                         (sparse_matrix.n_nonzero_entries() *
                            (sizeof(int) + sizeof(Number)) +
                          dst.size() * 3 * sizeof(Number) +
                          sizeof(std::size_t)) /
                         time
                    << " GB/s" << std::endl;
      }

      {
        ...
        const auto t1 = std::chrono::steady_clock::now();
        const auto info =
          solve_with_conjugate_gradient(500, 1e-12, sparse_matrix, dst, result);
  ...
  else
    {
      if (N_x == -1)
        for (unsigned long long NN = 24; NN < 500; NN += 24)
          {
            n_repeat = std::max(2ULL, 10000000000ULL / (NN * NN * NN));
            run_test<float>(NN, NN, NN, n_repeat, do_sparse);
          }
      else
        {
          run_test<float>(N_x, N_y, N_z, n_repeat, do_sparse);
        }
    }
