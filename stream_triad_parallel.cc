...
// gcc -O3 -march=native -fopenmp stream_triad_parallel.cc -o stream_triad_parallel
// and run as "./stream_triad_parallel 100 0
...
// right thread
double *
get_aligned_vector_pointer(const unsigned int N, std::vector<double> &x)
{
                     ...
                     const bool test_multiple_vectors)
{
...
#endif
  const unsigned int local_size = ((N_wish + n_threads - 1) / n_threads + 7) / 8 * 8;
  ...
  if (test_multiple_vectors == false)
    {
      // Case where we work on a single long vector. This is pretty slow on multiple
      // threads because we need to start up loop of small size and because we
      // have NUMA conflicts when different cores work on nearby memory regions
      // (same memory page)
...
#pragma omp parallel for
              for (unsigned int i=0; i<n_threads; ++i)
                {
                  for (unsigned int rep=0; rep<n_repeat; ++rep)
                    compute_daxpy(local_size, 13, v1+i*local_size,
                                  v2+i*local_size, v3+i*local_size);
                  const volatile double result = v3[0];
                  // prevent compiler to report about an unused variable
                  (void)result;
                }
...
#pragma omp master
  ...
  std::cout << "daxpy of size "
            << std::setw(8) << N
            << " : min/avg/max: "
            << std::setw(11) << best << " "
            << std::setw(11) << avg/n_tests << " "
            << std::setw(11) << worst << " seconds or "
            << std::setw(8) << 1e-6 * N / best << " MUPD/s or "
            << std::setw(8) << 1e-9 * 3 * 8 * N / best << " GB/s"  << std::endl;
}
